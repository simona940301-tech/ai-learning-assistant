...
pub async fn start_lobby_confirm_timer(
    ...
    loop {
        ...
        let (should_continue, both_confirmed) = {
            ...
            if let Some(match_record) = matches_guard.get_mut(&match_id) {
                ...
                if both_confirmed {
                    ...
                    info!("Match {} started, state changed to InBattle (PVE: {})", match_id, is_pve);
        ...
        if !should_continue {
            if both_confirmed {
                // 雙方都已確認，發送確認完成消息
                info!("Lobby {} confirmed by both players", match_id);
                send_lobby_confirmed(&match_id, &players, &connections, &matches).await;
            } else {
                // 計時器結束或 match 不存在
                if countdown <= 0 {
                    // 超時，解散房間
                    info!("Lobby {} timeout, dissolving", match_id);
...
async fn send_lobby_confirmed(
    ...
    let question_list = {
        ...
        if let Some(match_record) = matches_guard.get(match_id) {
            ...
            warn!("Match {} not found when sending LOBBY_CONFIRMED", match_id);
            return;
        }
    };
    ...
    for player_id in players {
        if let Some(tx) = connections_guard.get(player_id) {
            ...
            if let Err(e) = tx.send(confirmed_message) {
                warn!("Failed to send LOBBY_CONFIRMED to {}: {}", player_id, e);
            }
            ...
            if let Err(e) = tx.send(match_found_message) {
                warn!("Failed to send MATCH_FOUND to {}: {}", player_id, e);
            }
        }
    }
