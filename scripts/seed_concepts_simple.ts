...
async function seedConcepts() {
  try {
    console.log('Starting concept seeding (simple version)...')
    ...
    // Process concepts without embeddings
    let successCount = 0
    let errorCount = 0
    ...
    for (const concept of conceptsData) {
      try {
        ...
        // Upsert concept without embedding
        const { error } = await supabase
          .from('concepts')
          .upsert({
            ...concept,
            embedding: null, // Will be null for now
