...
import { ArrowRight, Sparkles, Crown, RefreshCw } from 'lucide-react';
...
interface FlowControlCardProps {
    ...
    isExit?: boolean; // 🎯 New: Exit mode
}

export const FlowControlCard: React.FC<FlowControlCardProps> = ({
    masteredCount,
    reviewCount,
    onSwipe,
    onOpenCaptureModal,
    ...
    const overlayColor = useTransform(x, [-150, 0, 150], ['rgb(168, 85, 247)', 'rgba(0,0,0,0)', 'rgb(100, 116, 139)']); // Right: Grey for Exit
                ...
                {showTutorial && (
                    ...
                    >
                        ...
                        <div className="absolute left-[-20%] top-1/2 -translate-y-1/2 flex flex-col items-center animate-pulse">
                            <div className="bg-gradient-to-r from-purple-500 to-pink-500 text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-lg mb-2 whitespace-nowrap">
                                {isExit ? '收藏結束' : '捕獲生字'}
                            </div>
                            <ArrowRight className="text-purple-500 rotate-180 w-8 h-8 filter drop-shadow-md" />
                        </div>
                        {/* Right Hint */}
                        <div className="absolute right-[-20%] top-1/2 -translate-y-1/2 flex flex-col items-center animate-pulse">
                            <div className={cn(
                                "text-white text-xs font-bold px-3 py-1.5 rounded-lg shadow-lg mb-2 whitespace-nowrap",
                                isExit ? "bg-zinc-500" : "bg-cyan-500"
                            )}>
                                {isExit ? '離開' : 'Swipe Right to Continue'}
                            </div>
                            <ArrowRight className={cn("w-8 h-8 filter drop-shadow-md", isExit ? "text-zinc-500" : "text-cyan-500")} />
                        </div>
                ...
                <div className="relative w-full h-full flex flex-col p-8 items-center justify-between z-20">
                    ...
                    <div className="w-full flex justify-between items-center text-zinc-500 dark:text-zinc-400">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-bold tracking-widest uppercase">
                                {isExit ? 'Game Paused' : 'Session Recap'}
                            </span>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col items-center text-center space-y-8">
                        ...
                        <div className="grid grid-cols-2 gap-4 w-full">
                            <div className="bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-amber-900/10 p-5 rounded-2xl border border-amber-100 dark:border-amber-800/30">
                                ...
                                <div className="text-[10px] font-bold text-amber-600/60 dark:text-amber-400/60 uppercase tracking-widest">
                                    {isExit ? '本次生字' : 'To Review'}
                                </div>
                            </div>
                            <div className="bg-gradient-to-br from-green-50 to-emerald-50 dark:from-emerald-900/20 dark:to-emerald-900/10 p-5 rounded-2xl border border-emerald-100 dark:border-emerald-800/30">
                                ...
                                <div className="text-[10px] font-bold text-emerald-600/60 dark:text-emerald-400/60 uppercase tracking-widest">
                                    {isExit ? '已熟悉' : 'Mastered'}
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="w-full grid grid-cols-2 gap-4">
                        ...
                        <div className="flex items-center gap-3 text-zinc-400 dark:text-zinc-500 group">
                            ...
                            <span className="text-xs font-bold uppercase tracking-wider group-hover:text-amber-500 transition-colors">
                                {isExit ? '收藏/結束' : 'Review'}
                            </span>
                        </div>
                        ...
                        <div className="flex items-center justify-end gap-3 text-zinc-400 dark:text-zinc-500 group">
                            <span className="text-xs font-bold uppercase tracking-wider group-hover:text-zinc-600 dark:group-hover:text-zinc-300 transition-colors">
                                {isExit ? '直接離開' : 'Continue'}
                            </span>
                            <div className="p-2 rounded-full bg-zinc-100 dark:bg-zinc-800 group-hover:bg-zinc-200 dark:group-hover:bg-zinc-700 transition-colors">
                                <ArrowRight className={cn(
                                    ...
                                    isExit ? "group-hover:text-zinc-600" : "group-hover:text-indigo-500"
                                )} size={18} />
                            </div>
                        </div>
                    </div>
