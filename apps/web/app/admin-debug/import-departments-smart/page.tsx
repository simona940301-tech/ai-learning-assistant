...
export default function ImportDepartmentsSmartPage() {
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      ...
      if (apiKey) {
        headers2['x-internal-api-key'] = apiKey
      }
...
國立台灣大學,機械工程學系,004112,11,無,均標,頂標,前標,--,--,前標,A`
  ...
  return (
    <div className="container mx-auto max-w-5xl p-6 space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold mb-2">智慧匯入校系標準 (Debug版)</h1>
      ...
      <Card>
        <CardHeader>
          <CardTitle>API Key (開發用)</CardTitle>
          <CardDescription>輸入 INTERNAL_API_KEY 以繞過認證</CardDescription>
        </CardHeader>
        <CardContent>
          <input
            type="password"
            placeholder="請輸入 INTERNAL_API_KEY"
            value={apiKey}
