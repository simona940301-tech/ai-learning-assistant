-- 測試腳本：驗證羽毛倒數計時器不會重置
-- 執行此腳本後，重新整理頁面多次，倒數計時器應該保持一致

-- 1. 設置測試場景：用戶在 1 小時前消耗了羽毛
-- 預期：應該會看到還剩約 29 分鐘才能恢復下一根羽毛

-- 替換成你的用戶 ID
\set user_id '8b9f3f4c-13c0-4319-9eaa-a5ca55973bcc'

-- 模擬 1 小時前消耗羽毛（從 8 → 7）
UPDATE profiles
SET
  daily_energy_count = 7,
  energy_last_updated_at = NOW() - INTERVAL '1 minute'  -- 1 分鐘前消耗
WHERE id = :'user_id';
