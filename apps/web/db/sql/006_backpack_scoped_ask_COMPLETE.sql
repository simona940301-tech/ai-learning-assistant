-- ============================================
-- Backpack Scoped Ask - 完整 SQL 匯入文件
-- ============================================
-- 
-- 執行順序：
-- 1. 先執行 005_backpack_upload.sql（如果尚未執行）
-- 2. 再執行此文件（006_backpack_scoped_ask.sql）
--
-- 在 Supabase SQL Editor 中執行此文件
-- ============================================

-- ============================================
-- 前置檢查：確保必要的 extensions 已啟用
-- ============================================
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";
CREATE EXTENSION IF NOT EXISTS "vector";

-- ============================================
-- 0) 擴展 doc_chunks：加入 start/end（字元位移）
-- ============================================
ALTER TABLE public.doc_chunks
  ADD COLUMN IF NOT EXISTS start INT,
  ADD COLUMN IF NOT EXISTS "end" INT;

-- 索引（page + start 有助於高亮映射）
...
--
-- 已擴展的表：
-- - doc_chunks（新增 start/end 欄位）
--
-- 已建立的函數：
-- - search_doc_chunks_scoped（範圍內向量檢索）
--
-- 所有表都已啟用 RLS（Row Level Security）
-- ============================================
